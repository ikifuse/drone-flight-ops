...
suite('旧運航UIの継承と保存失敗',H=>{
 ...
 T('BAT交換を戻ると直前の装着BATを保持し、着陸後の選択へ戻る',()=>{const before=JSON.stringify(A().op.cur);act('op-to-bat');act('op-bat-cancel');return A().route==='op-next'&&JSON.stringify(A().op.cur)===before});
