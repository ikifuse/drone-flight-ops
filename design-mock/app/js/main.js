...
const SCREEN_MAP=[
  ['はじめの登録・会社団体',[['init-reg','はじめの登録（Googleアカウント・本人情報・DIPS情報）'],['create-name','会社・団体の名前'],['join1','参加する会社・団体'],['join2','参加の確認'],['join3','あなたの名前を選んでください'],['where','どこで使いますか？']]],
  ['ホーム',[['home','ホーム（4入口）']]],
...
const PRE_ENV=['init-reg','create-name','join1','join2','join3','where'];
function gotoScreen(id){
  A.modal=null;A.mockModal=null;const E=ENV();
  const base=id.split(':')[0];
  if(!PRE_ENV.includes(base)&&!E){mtoast('まず個人か会社・団体で使い始めてください');render();return}
  if(PRE_ENV.includes(base)&&base!=='init-reg'&&!A.account)A.account=ACCOUNTS.find(a=>a.id===A.ui.gState)||ACCOUNTS[0];
