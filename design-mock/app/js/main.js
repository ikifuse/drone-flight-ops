...
const SCREEN_MAP=[
  ['はじめて使う・ログイン',[['boot','はじめに（利用登録を始める／ログイン）'],['gauth','Googleアカウントを選択します（Googleが表示）'],['acct-exists','すでに登録されています'],['acct-none','ログインできませんでした'],['usage','どのように使いますか？'],['create-name','会社・団体の名前'],['consent','Google Driveの許可（Googleが表示）'],['join1','招待を受けている会社・団体'],['join2','参加の確認'],['join3','あなたの名前を選んでください'],['where','どこで使いますか？']]],
  ['ホーム',[['home','ホーム（4入口）']]],
...
const PRE_ENV=['boot','gauth','acct-exists','acct-none','usage','create-name','consent','join1','join2','join3','where'];
...
function controlsHtml(){
  ...
  return '<h4>設計確認用の状態切替（実際のアプリにはありません）</h4>'
   ...
   +'<p class="note" style="margin:6px 0 0">ホームへ入る前の一括の初期設定はありません。機体・操縦者・DIPSのログイン情報などは、［各種設定・管理］から先に登録するか、必要になった場面でその場で登録します（34a §7.8）。</p>'
   +'<h4>そのほかの操作</h4>'
