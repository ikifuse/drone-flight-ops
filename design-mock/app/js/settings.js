...
def('set-bat',{t:'BAT管理',st:'BATの一覧',
  ...
  tmp:['32dは「表示の案（オーナー確認待ち）」。現在状態（保管・充電など）の値は未確定で、ここには出していない（PENDING-D-BAT-STATES）','BATを新しく登録する画面は個別には設計されていない。ここでは登録の入口だけ置いた（案）','件数が多いときの見せ方は未検証（VERIFY-D-BAT-SCALE）'],
...
def('set-docs',{t:'許可・承認／保険／連絡先',st:'DIPSの入力に自動で入る情報',
  ...
  tmp:['3つを1画面にまとめる分類は案','許可承認は「複数」、保険と連絡先は「1つ」としているのも案'],ask:['3つを別々の設定項目にするか、まとめるか'],
  body:()=>{
    const E=ENV();
...
def('set-presets',{t:'現場プリセット',st:'よく行く現場の範囲・高度',
  ...
  tmp:['一覧と登録画面は案。プリセットは新規飛行の飛行範囲の画面からも保存できる想定'],ask:['現場（場所）と範囲プリセットを、別の設定項目に分けるか'],
  body:()=>{
    const E=ENV();
...
def('set-maint',{t:'点検整備記録',st:'機体ごとの詳しい点検整備',
  ...
  tmp:['入口だけ置いた案の画面。中身は未設計'],ask:['点検整備記録を、設定の中に置くか、機体の詳細から開くか'],
