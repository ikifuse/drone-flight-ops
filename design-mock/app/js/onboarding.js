...
def('init-reg',{t:'はじめの登録',st:'',env:false,back:false,
  ...
  tmp:['DIPSログイン情報の保存先・暗号化・端末ごとか共有か・複数人が使うときの閲覧・自動ログインに使うか・API認証との関係・削除や無効化は未決（PENDING-S5-DIPS-LOGIN-STORAGE）','このモックは入力した内容を保存も送信もしない。DIPSパスワードの文字は、登録した時点で消している。本物のID・パスワードは入れないこと','住所・都道府県・国などの項目の分け方は、DIPSの連絡先の項目に合わせるところまでで、最終の列は未確定','ここで登録した人物を、そのまま操縦者として扱うかは未決（役割は人物情報とは別に持つ設計のため、いまは自動で操縦者にしていない）'],
  ask:[],
