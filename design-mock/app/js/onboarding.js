...
def('init-reg',{t:'はじめの登録',st:'',env:false,back:false,
  ...
  doc:'34a §9.2・§9.3（2026-09-23同日中に改訂。必須は氏名とGoogleアカウントの2つ。残りは任意）／§9.5（登録した人物情報を飛行計画で再利用する）／25b（DIPSの連絡先は自アカウント情報・操縦者から自動入力できる）／31a §2（人物とGoogleアカウントは別。ここで登録するのは人物の情報）。DIPSログイン情報を保存できる方針は34a §8.1（オーナー指示）。',state:'accepted',
...
def('create-name',{t:'会社・団体の名前',st:'',env:false,
  ...
  doc:'34a §4・§9.4（会社・団体の環境は、ホームから、その会社で使うGoogleアカウントで認証したあとに作る）／§9.3改訂（Google Driveの許可・保存場所の作成は、この画面を分けず内部処理する）。同名の保存場所がDrive上にすでにないか確認する（root重複防止・再発見の方式は未確定＝PENDING-S5-ROOT-DISCOVERY）。',state:'accepted',
