...
const dipsLabel=p=>p.dips==='dup'?'通報済み・重複あり':p.dips==='manual'?'通報確認済み（Manual）':'通報済み・重複なし';
...
def('list',{t:'飛行リスト',st:'これから扱う、通報済みの計画',
  ...
  ask:['カードの情報量（5系統だけで足りるか）','絞り込みの並べ方と初期選択','予定日時が過ぎた計画をどう見せるか（整理の条件は未確定）'],
  body:()=>{
    ...
    return '<p class="lead">通報が正常に受け付けられ、これから実飛行（または中止など）を扱う計画です。過去に完了した飛行は［飛行履歴・出力］にあります。</p>'+(total?chips:'')
     ...
     +'<p class="note">'+(A.online?'共有の作業リストを表示しています。':'オフライン: 端末に取得済みの内容を表示しています。最新とは限りません（鮮度の見せ方は未確定）。')+'</p>';
  }
});
def('plan',{t:'DIPS通報内容',st:()=>{const p=planOf(A.ui.planId);return p?p.name:''},
  ...
  tmp:['全表示項目はここで新規定義しない（既存の計画・提出記録を参照）。ここではDIPSの22項目と地図を、通報した内容として表示','日時・場所などの変更は、DIPS側の変更／再通報を伴う別処理（後続）。入口の見せ方は仮','中止／削除の確認画面・戻り方は未確定。DIPS取消は制度上可能な範囲で行う（24b §3）','重複ありの調整は未設計（PENDING-S7B-DUPLICATE-ADJUST）'],
  ask:['中止／削除を、この画面のどこに置くか（誤操作を避けたい）','通報内容の全項目を出すか、要約にするか'],
  ...
  body:()=>{
    ...
    const s=p.snap;
    return (p.dips==='dup'?'<div class="msg warn">他の計画と重複しています。調整の操作はまだ設計していません。</div>':'')
     +'<div class="msg info">これは、DIPSへ<b>通報した内容</b>です。通報した内容は、あとから黙って書き換えません。日時・場所を変えるときは、DIPS側の変更／再通報を伴う別の手続きになります。</div>'
     +'<div class="sec"><h3>通報の情報</h3><table class="kv"><tr><td>通報者</td><td>'+esc(plName(p.rep))+'</td></tr><tr><td>操縦者</td><td>'+p.pl.map(id=>esc(plName(id))).join('、')+'</td></tr><tr><td>DIPS状態</td><td>'+esc(dipsLabel(p))+'</td></tr><tr><td>KML</td><td>'+(p.kml==='saved'?'保存済み（07）':'未同期（端末に保持）')+'</td></tr></table></div>'
     ...
     +'<p class="note"><button class="chip" data-act="stub" data-t="日時・場所の変更（別処理）" data-m="通報した内容の変更は、DIPS側の変更／再通報を伴う別処理として扱います。入力・再通報・取消の全フローは、まだ設計していません（34c §4・24b §3）。">日時・場所を変えたい場合</button> '+tmpChip+'</p>';
  },
...
Object.assign(ACTS,{
  ...
  'plan-to-op':()=>{const p=planOf(A.ui.planId);if(p&&typeof startOp==='function')startOp(p,null);else openStub('作業途中','飛行前点検以降の画面は、このモックの次のコミットで追加します。')},
  'plan-cancel':()=>openSheet(()=>'<h3>この飛行を中止・削除しますか</h3><ul style="padding-left:1.2em;font-size:14px"><li>制度上可能な範囲で、DIPS側の取消を行います（DIPS側の取消は、アプリの外の手続きです）。</li><li>アプリ側では取消を記録し、この計画を<b>飛行リストから整理</b>します。</li><li>これまでの通報履歴・受付の証跡は、消えません。</li></ul><p class="note">事故・急病・通信不能などで、DIPS側の取消ができなかった場合でも、作業リストからは整理できる設計です（24b §3）。取消の入力・確認の画面は未確定です。</p><div class="row"><button class="btn" data-act="close">やめる</button><button class="btn danger" data-act="plan-cancel-ok">中止する</button></div>'),
  'plan-cancel-ok':()=>{if(!canWrite())return;const E=ENV();E.plans=E.plans.filter(p=>p.id!==A.ui.planId);A.ui.planId=null;A.modal=null;back();toast('計画を中止し、飛行リストから整理しました（通報の履歴は残ります）')}
});
