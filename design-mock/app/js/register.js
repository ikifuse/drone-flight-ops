...
const REG_META={
  ...
  insurance:{doc:'25b／26（DIPSの保険に関する情報：会社名・商品名・対人／対物の無制限／金額）。設定のどこに置くかは未確定（PENDING-S6-DRIVE-PLACEMENT）。',state:'tmp',
    tmp:['このモックは環境に1つだけ持つ簡略版。設計（12d §2）では保険は独立した保険台帳で、複数の契約を持て、組織・複数機材で再利用できる。飛行計画は適用する保険を1つ参照し、今回の値を上書きできる','「賠償能力」は保険加入と同一視できない項目のため、登録には含めない'],ask:[],ui:['入力欄の並びは標準案']},
