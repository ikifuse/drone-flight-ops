...
const MODELS=['EVO Lite+','EVO Lite','EVO Lite 6K Enterprise','Q3','X2','その他（手入力）'];
...
const FAKE_PLACES=['○○駅前','○○公園','○○川の河川敷','工場'];
...
const pnm=p=>p.name||'（名前未登録）';
const plName=id=>{const p=plOf(id);return p?pnm(p):(id||'—')};
...
function seedPersonal(env){
  ...
  env.presets.push(
   {id:'pr1',name:'河川敷Aの現場',geo:GEO.river,alt:30,from:'事務所',to:'河川敷A',biz:['空撮'],dur:[0,30]},
   {id:'pr2',name:'公園Bの定期点検',geo:GEO.park,alt:20,from:'事務所',to:'公園B',biz:['インフラ点検・保守'],dur:[1,0]});
  ...
  env.plans.push({id:uid('pl'),name:'河川敷Aの空撮（明日）',snap:v({name:'河川敷Aの空撮（明日）',ac:['a1'],pl:[P1],pm:'m1',biz:['空撮'],air:[AIR[3]],met:[MET[6]],geo:GEO.river,from:'事務所',to:'河川敷A',dur:[0,30],alt:30,startAt:at(1,10,0)}),
    start:at(1,10,0),place:'河川敷A',ac:['a1'],pl:[P1],rep:P1,dips:'clean',kml:'saved'});
...
function seedCompany(env){
  ...
  env.people.push(
   newPerson('会社事務担当',['管理者'],{id:'q0',account:'office.admin@example.invalid'}),
  ...
  env.batGroups.push({id:'g2',name:'Q3のBATグループ',models:['Q3']});
  env.bats=mkBats('g2','型式Y',1,[['異常なし',20,'今週',300,25,0,'c1'],['異常なし',null,null,210,17,2,'c1'],['異常なし',8,'先月',90,9,9,'c1']]);
  const v=o=>snapOf(env,o);
  const P=(name,d,h,ac,pl,rep,dips,o)=>({id:uid('pl'),name,snap:v(Object.assign({name,ac,pl,pm:'n1',biz:['インフラ点検・保守'],air:[AIR[3]],met:[MET[6]],geo:GEO.park,from:'本社',dur:[1,0],alt:25,startAt:at(d,h,0)},o)),start:at(d,h,0),place:o.to,ac,pl,rep,dips,kml:'saved'});
  env.plans.push(
   P('工場の屋根点検',1,10,['c1'],[P1],'q0','clean',{to:'工場'}),
   P('現場の進捗空撮',2,14,['c2'],['q2'],'q0','clean',{to:'現場',biz:['空撮']}),
...
function samplePersonalEnv(acc){
  ...
  const me=newPerson('操縦者A',['管理者','操縦者'],{id:'p1',account:acc.email,phone:'09000000000'});
  env.people.push(me);env.meId='p1';seedPersonal(env);return env;
}
function sampleCompanyEnv(name,kind,meId,acc){
  const env=emptyEnv(name||'○○株式会社',kind||'company');
  seedCompany(env);
...
/* 設計確認用（右側だけで使う）: Google認証のあとの状態と、その状態で使う架空のGoogleアカウント。左側の画面には出さない */
const ACCOUNTS=[
  {id:'new',email:'test.new@example.invalid',state:'未登録（はじめての人）'},
