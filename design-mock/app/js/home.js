...
def('home',{t:'ホーム',back:false,
  ...
  tmp:['ホームの10項目は34b §2。4入口と環境シートへの入口を維持する','現在の場所は上の［〜で使用中］で表示し、複数ある場合はこのボタンから切り替える','空の状態のときのひとこと案内は案。左側の説明は最小にしている（機体・操縦者などは飛行の途中でもその場で登録できることは、ここには書いていない。案内が要るかは未決）','DIPSのログイン情報が未登録のときの注意表示をホームに出すかは未決（いまは出していない）'],
  ask:[],
  ui:['初めて使うときのひとこと案内は、空のときだけホームに出している','使っている場所は上に「〜で使用中」として出し、会社・団体の追加はこのボタンで開くシートに置いている','別の［切り替える］は置かず、上の［〜で使用中］から現在地確認・会社追加・切替を行う','左側の言葉は「会社・団体で新しく使い始める」「すでに使っている会社・団体に参加する」にしている。内部では、前者が新しい運用環境の作成、後者が既存の運用環境への所属の追加（34h §3のとおり「環境」を画面に出さない）'],
  body:()=>{
