...
| 対象 | 詳細正本 |
|---|---|
| PENDING-S5-ROOT-DISCOVERY / INITIAL-REQUIRED | [34a](architecture/presentation/34a_setup-and-environment-entry.md)：root再発見方式、参加・中断復旧、必須範囲。2026-09-23に、**初回は全員が個人環境から始め、本人情報（氏名・フリガナ・住所・電話番号・メールアドレス）とDIPSログインID・パスワードを1画面で登録してからホームへ入る**ことを確定した（[§9](architecture/presentation/34a_setup-and-environment-entry.md#9-初回は個人環境から始め初回登録を1画面にまとめる2026-09-23)・[ADR-0030](decisions/ADR-0030-personal-first-onboarding-and-single-screen-initial-registration.md)）。INITIAL-REQUIREDの残りは、**初回登録の7項目のうち何を必須にするか**（現在は氏名のみ）と、**飛行を始めるときに何を必須にするか**（最低1機案を含む）。機体・許可承認・保険の事前登録は［各種設定・管理］から、不足は必要になった場面で案内する |
