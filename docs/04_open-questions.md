...
| 対象 | 詳細正本 |
|---|---|
| PENDING-S5-ROOT-DISCOVERY / INITIAL-REQUIRED | [34a](architecture/presentation/34a_setup-and-environment-entry.md)：root再発見方式、参加・中断復旧、機体／BAT等の必須範囲。2026-09-22に、**ホームへ入る前には何も必須にしない**ことを確定し（[§7.8](architecture/presentation/34a_setup-and-environment-entry.md#78-ホームへ入る前を最小にした訂正2026-09-22)）、INITIAL-REQUIREDの残りは「飛行を始めるときに何を必須にするか（最低1機案を含む）」に縮小した。事前登録は［各種設定・管理］から、不足は必要になった場面で案内する |
| PENDING-S5-DIPS-LOGIN-STORAGE | [34a §8](architecture/presentation/34a_setup-and-environment-entry.md#8-dipsログイン情報の登録と初回設定の候補2026-09-22)・[16 §10](architecture/16_security.md#10-利用者自身のdipsログイン情報2026-09-22方針のみ)：DIPSログインID・パスワードの保存先・暗号化方式・端末ごと／共有・Google Drive／Sheetsでの持ち方・複数人利用時の閲覧・自動ログインに使うか・API認証との関係。**方針は「アプリに登録・保存する」（オーナーの指示）で、AIの判断で変えない**。詳細は未決 |
| PENDING-S5-ENTRY-SCREENS | [34a §7.5](architecture/presentation/34a_setup-and-environment-entry.md#7-利用者向けの表示名と初回導線の訂正2026-09-21)：アカウントから始める新しい入口の画面（1画面の入口［利用登録を始める］／［ログイン］、使い方の選択、どこで使いますか？）ごとの10項目。2026-09-22に、説明専用画面を廃止して入口を1画面へまとめた（[34a §7.6](architecture/presentation/34a_setup-and-environment-entry.md#76-初回の入口を1画面にまとめた訂正2026-09-22)）。ログイン時に登録済みかを判定する方法はPENDING-S5-ROOT-DISCOVERYに依存 |
