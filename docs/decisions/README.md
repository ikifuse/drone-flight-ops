...
| [ADR-0030](ADR-0030-personal-first-onboarding-and-single-screen-initial-registration.md) | 初回は全員が個人環境から始め、氏名とGoogleアカウントだけを必須に1画面で登録する（残りは任意。不足は通報時に不足分だけ補い、人物情報とDIPSログイン情報へ分けて保存）。会社・団体はホームから、その会社で使うGoogleアカウントで追加する。Google認証・許可・環境作成は独立画面にせず内部処理にする | **提案中（Proposed）** | 未承認（記録2026-09-23、同日改訂） | 0016をClarifies。34a §7.7の3択画面と§7.8の「本人情報・DIPS情報をホーム前から外す」部分をPartially Supersedes。詳細は34a §9 |
