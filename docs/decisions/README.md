...
| [ADR-0030](ADR-0030-personal-first-onboarding-and-single-screen-initial-registration.md) | 初回は全員が個人環境から始め、氏名とGoogleアカウントだけを必須に1画面で登録する（残りは任意。不足は通報時に不足分だけ補い、人物情報とDIPSログイン情報へ分けて保存）。会社・団体はホームの環境シートから、その会社で使うGoogleアカウントで追加する。初回二択入口は廃止、単一環境でも環境ボタンは有効。Google認証・許可・環境作成は独立画面にせず内部処理にする | **提案中（Proposed）** | 未承認（記録2026-09-23、同日改訂） | 0016をClarifies。34a §7.7の3択画面と§7.8の「本人情報・DIPS情報をホーム前から外す」部分をPartially Supersedes。詳細は34a §9 |

ADR-0010〜0014は整理前の旧main（`6344d7a`。既存タグ`backup/main-6344d7a-20260919`で参照可能。当時のバックアップbranchは2026-09-22に削除済み）にProposedとして存在する。本線はその作成前の`ea73d08`から再移植しており、旧Step成果をコピーしないため収録しない（対応関係は[Git本線の整理](../migration/99-2-git-mainline-cutover.md)）。番号を再利用せずStep 1で0015、Step 2で0016、Step 3で0017、Step 4で0018、Step 5で0019、Step 6で0020〜0022、Step 7aで0023、Step 7cで0024、Step 7dで0025、Step 7eで0026・0027、2026-09-20のBAT管理設計の更新で0028・0029を、2026-09-23の初回利用フローの確定で0030を新設した。旧mainの0010〜0014をAcceptedへ変更していない。
...
| 0007 §2.8全面的タブ禁止・§2.9整備サマリーと自動続紙 | 現在99.2と最新A4直接確認に基づく限定置換を[0021](ADR-0021-a4-record-layout-and-sheet-boundary.md)／[0022](ADR-0022-drive-responsibilities-and-human-records.md) Proposedへ記録。現行詳細は35c／36／37。BATの人間向け媒体への拡張は[0028](ADR-0028-battery-storage-by-shareable-aircraft-family.md)。Accepted本文の承認履歴は保持 |
