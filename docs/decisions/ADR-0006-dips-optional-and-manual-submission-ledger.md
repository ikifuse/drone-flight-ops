...
また、国交省DIPSサーバー側のデータ保持期間や将来の参照可能期間は利用規約上保証されておらず、DIPS側にしか履歴が残らない設計は自社の法令遵守・運航管理の観点から脆弱である。
...
- **選択肢C: API非依存・手動第一級サポート・提出スナップショット＆独立台帳保持（推奨案）**:
  - DIPS通報前に、提出予定内容の不変スナップショット（`DipsSubmission`）を生成し、Googleスプレッドシート「DIPS飛行計画台帳」へ先行保存。
  - 手動通報を「正式な第1級機能」として扱い、専用の手動入力支援画面（1タップコピーUI）と明確な通報打刻フローを実装。
  - 「操縦者による手動通報記録」と「DIPS側の受付番号確認」を厳格に区別する状態マシンを確立。
  - DIPS APIは「同一データモデル上で最後の送信経路だけを切り替えるOptional Integration」とし、APIがなくてもPhase Cで100%完成できるロードマップを採用。
...
1. **Googleスプレッドシート「DIPS飛行計画台帳」の論理保持**:
   - DIPSへの通報（API/手動問わず）前に、提出予定内容を自社台帳へ先行退避する。
   - DIPSサーバー側の保存期間に依存せず、過去の全通報履歴・改訂履歴を自前で半永久保持する。
2. **不変スナップショット（`DipsSubmission`）による履歴保護**:
   - 計画が変更・再通報されても、過去に通報した内容は上書き破壊せず、新リビジョンとして追記し、`supersedes_submission_id` で改訂履歴を追跡する。
3. **手動入力支援画面（1タップコピーUI）の実装**:
   - スマホ1台で現場運用できるよう、DIPS入力に必要な全項目を整理表示し、1タップでクリップボードへコピー可能とする。
4. **状態管理における誤認防止（5大ルール）の徹底**:
   - 「台帳保存済み」≠「DIPS通報済み」
   - 「手動通報記録」≠「DIPS確認済み」
   - 「API受理確認」と「手動受付番号確認」の区別
   - 「Mock成功」≠「本番通報成功」
   - 「通報完了」≠「飛行可能」
5. **Phase C実装ロードマップの非依存化**:
   - Phase C6で手動通報運用を含むアプリ本体を完成させ、Phase C7（実API中継）は承認時のみ着手するOptional扱いとする。
...
- **メリット**:
  - DIPS APIの審査結果（承認・保留・却下）に関わらず、現場で100%実用可能なアプリとして確実に完成できる。
  - 将来APIが承認された場合でも、データベースや台帳を作り直すことなく、`ApiDipsAdapter` を追加するだけでシームレスに自動化へ昇格できる。
  - 国交省サーバー停止時や完全電波圏外でも、自社台帳とローカルDBに過去の通報履歴が完全保全される。
- **デメリット・留意点**:
  - 手動通報時はパイロット自身によるDIPS画面への転記と受付番号の入力操作が必要となる（専用支援画面により負荷を極小化）。
