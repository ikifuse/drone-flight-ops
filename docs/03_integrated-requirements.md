...
### 2.2 飛行ごとのバッテリー紐付け記録
- 各飛行（離陸〜着陸）において、使用したバッテリー個体IDを必須記録（BAT管理がONの機体）。
- 飛行前残量（%）、飛行後残量（%）。（現在の現場入力4項目に含まれない。扱いは[32g](architecture/asset-management/32g_battery-field-input.md)のPENDING-D-BAT-LEGACY-CHECKS）
...
- 装着・確実なロックの確認、残量・温度・警告表示の有無の最小チェック（3ステップ）を経て、素早く次の離陸待機へ進めること（BAT使用開始・交換時の人の入力は現在4項目。[32g](architecture/asset-management/32g_battery-field-input.md)のPENDING-D-BAT-LEGACY-CHECKS）。同一帳票内で飛行記録が継続される。
...
- **通報エラーハンドリング**: DIPS側から返るエラーレスポンスを解析し、修正項目をユーザーに平易に提示。画面に出す文言は、内部用語を使わず、問題・データ保護状態・次の操作の型で示す（[30 §4](architecture/presentation/30_screen-specification-standard.md#4-内部設計用語と利用者表示文言の分離)、[34h](architecture/presentation/34h_user-facing-wording-and-terminology.md)）。
...
- **地図付き飛行計画書PDF**: 飛行範囲の地図画像、飛行空域座標、連絡先、許可承認番号をまとめた関係者提出用帳票。現在は、DIPS通報内容とGeometryの地図を持つ地図付きPDFとして、必要な時だけ生成する位置づけ（[27f](architecture/output/27f_derived-pdf-roles-and-map-pdf.md)。旧記述との関係は未確定）。
