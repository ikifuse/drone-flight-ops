...
§1〜§7は2026-09-19当時の判断・作業・検査の記録（HISTORICAL）。当時の「現在」「今後」やbranch名は現行の作業指示ではない。2026-09-22の現在状態と訂正は§8を参照する。
...
- 旧mainの内容・履歴を見る: `backup/main-6344d7a-20260919`を開く。当時併用したbranch `archive/main-before-99-2-redo-20260919`は2026-09-22に削除済み（§8）。
...
## 8. mainへの集約と現在状態の訂正（2026-09-22）

設計変更ではなくGit運用の整理。オーナー指示により、`design/new-flight-mock-20260921`の最新`d020e3e0b2fd18a05d037a98dcb4885dc61cbc4f`を、既存16 commitを保持するfast-forwardでmainへ反映した。両HEAD・内容の一致を確認した後、mainに含まれない独自commitが0件である次の7本をGitHub上で削除した。

- `archive/main-before-99-2-redo-20260919`
- `claude/99-2-continuation`
- `design/battery-management-20260920`
- `design/claude-code-routine-20260920`
- `design/environment-aircraft-battery-group-20260920`
- `design/new-flight-mock-20260921`
- `redo/99-2-causal-migration`

削除後、GitHub上のbranchはmainの1本だけで、削除操作によるmainのHEAD変更はなかった。commit履歴を書き換えず、旧mainは履歴と既存タグ`backup/main-6344d7a-20260919`（対象commit `6344d7a`）で引き続き参照できる。タグは削除していない。個別の移植監査にあるbranch名・保全条件・HEADは当時の記録で、現在のbranchの存在を示さない。

§6の「継続branchで作業してmainへ反映する」運用は、この日のオーナー指示で置き換えた。今後の恒久ルールは[AGENTS §5](../../AGENTS.md#5-git作業完了の恒久ルール)を正本とし、本記録へ複製しない。実装開始の許可やアプリ設計の変更ではない。
