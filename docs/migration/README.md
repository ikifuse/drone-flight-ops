...
99.2全文や旧Step 1〜8の移植結果をコピーしない。旧main（整理前の`6344d7a`）の成果は比較証拠として保存し、再移植元にはしない。

各監査記録の「main」「比較用main／origin/main（`6344d7a…`）」は、2026-09-19の[Git本線の整理](99-2-git-mainline-cutover.md)前の旧mainを指す。当時の記録として書き換えていない。

各監査のbranch名・作業先・保全指示は当時の履歴。2026-09-22のmain集約とGitHub branch削除後の状態は[Git本線の整理 §8](99-2-git-mainline-cutover.md#8-mainへの集約と現在状態の訂正2026-09-22)を参照する。削除済みbranchでの作業を指示するものではない。
