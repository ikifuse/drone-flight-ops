...
- **99.2再移植はStep 3（§4と§6の取得確認・点検整備Actor部分）まで**。Step 1（§0・§1）・Step 2（§2）を保持し、人物領域は[identity-and-access](architecture/identity-and-access/README.md)、機材の取得・共用・履歴は[asset-management](architecture/asset-management/README.md)を詳細正本とする。[移植記録](migration/README.md)から確認範囲へ進む。§3・§5・§6の残り・§7以降は未移植で、既存詳細を99.2全体との監査済み仕様と扱わない。依存実装は[23の開始ゲート](architecture/23_implementation-roadmap.md#12-保存出力を確かめてから依存実装へ進むゲート)に従う。
...
| [移植記録README](migration/README.md) | 99.2再移植の対象・根拠・移管先・検査 | 監査記録 / Step 1〜3 |
...
│   └── ADR-0001〜0009 / 0015〜0017 # 既存決定と再移植のProposed記録（0010〜0014はmainに保全）
└── architecture/
