...
| [architecture/27_output-kml-drive-and-mymaps.md](architecture/27_output-kml-drive-and-mymaps.md) | **出力・KML・Google Drive・My Maps連携設計** | 4大出力連携境界（Sheets台帳/PDF帳票/KML/JSON内部通信のみ）、1計画1KML、Drive自動保存、My Maps手動連携、機体ログ重畳 | Geo Export実装時、Drive連携時、My Maps運用時 | **確定** | kml, google-drive, my-maps, geo-export, output |
...
└── architecture/                      # Phase B 設計書群
    ...
    └── 10_system-boundaries.md 〜 27_output-kml-drive-and-mymaps.md (Phase B2詳細設計・DIPS詳細・実画面検証・GeoExport)
```
