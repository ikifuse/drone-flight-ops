...
**BATの台帳（2026-09-20）**: No.6「バッテリー台帳」・No.7「バッテリー使用履歴」は旧の論理台帳案（HISTORICAL）。人が読むBATの保存は、共用機体系ごとの1Spreadsheet・1物理BAT＝1シート・総合台帳なしの[32f](../asset-management/32f_battery-storage-structure.md)に従い、独立した総合BAT台帳を正本として置かない。内部の同期対象としてのBAT個体マスターとの対応はPENDING-D-BAT-AUTHORITY-MAP（32f）。
