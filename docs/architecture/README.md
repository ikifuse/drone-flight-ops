...
| 初回導線（アカウントから始める）・作成／参加・root再発見・通常起動・DIPSログイン情報の登録方針（保存方式は未決） | [34a](presentation/34a_setup-and-environment-entry.md)（§8はDIPSログイン情報。秘密情報の境界は[16 §10](16_security.md#10-利用者自身のdipsログイン情報2026-09-22方針のみ)） | 人物・環境・権限は31a〜31d。表示名は34h。物理方式は未確定 |
