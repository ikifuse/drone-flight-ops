...
## 6. 個人本人の既知Googleアカウントを再入力させない（2026-09-24）

**発端・旧状態（HISTORICAL）**: 初回の個人環境作成では本人Personと認証アカウントを対応付けていたが、人員の共通フォームは本人にも「Googleアカウント（任意）」のメール入力欄を表示していた。本人が分かっている場面で同じアカウントを再入力させ、空欄保存によって既存の対応を失う余地があった。

**CURRENT-ACCEPTED（2026-09-24のオーナー追加指示）**: 個人環境の本人登録・本人編集では、その個人環境でログインしている既知のGoogleアカウントを本人Personへの対応として自動初期化し、保存後も対応を維持する。Googleアカウントのメールアドレスを再入力させない。これは、人物とGoogleアカウントの分離を維持したまま行うUI上の初期化・自動紐付けであり、「Googleアカウント＝人物」というモデルへの変更ではない。

本人の判定は現在の個人環境の本人Personへの対応で行い、氏名や連絡用メールの一致だけから推定しない。既存の本人がいるときはそのPersonを再利用し、登録経路ごとに本人を重複作成しない。連絡用メールはGoogle認証アカウントとは別情報であり、今回の紐付けのために入力を必須にしない。

本人以外の人員、会社・団体環境の人員（その環境の本人を含む）は従来どおりGoogleアカウント任意とする。アカウントを持たない補助者・点検者等も登録できる。会社用Google／Workspaceアカウント、初回Googleアカウント選択、同一人物の複数アカウント対応の設計は変更しない。

**維持する境界**: 認証や自動紐付けだけで操縦者役割を付与しない。操縦者登録済みの個人本人の新規飛行初期選択は[31c §6](31c_operational-actors.md#6-個人環境の本人操縦者を新規飛行の初期値にする2026-09-24)のまま。画面の対象切替・欄の表示・保存／取消は[34i](../presentation/34i_person-registration-and-account-linking.md)へ分離する。モックのmeId・accountは確認用で、人物ID・アカウントID・所属の物理schema、本番認証・失効時の再認証、権限・offline保存等のPENDING／VERIFYは解消しない。既知の認証アカウントが取得できない場合に手入力から認証済みの対応を作る仕様は追加しない。
