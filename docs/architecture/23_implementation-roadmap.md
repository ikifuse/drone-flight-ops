...
**現在の停止位置**: C0受入確認・オーナーGO待ち。オーナーが明示的に「実装開始」と指示するまで実装は凍結し、設計検討を継続する。99.2再移植はStep 1（§0・§1）、Step 2（§2）、Step 3（§4と取得確認・点検整備Actorに直接必要な§6の部分）、Step 4（§7のDIPS API基盤・固定IP・通信境界のみ）、Step 5（§3と§7の正常受付後・共有リストに必要な限定部分）、Step 6（§5・§6残り・§10）、Step 7a（§7の実画面確認・共通Geometry・submission_snapshot・Manual／API経路・DIPS対象外）、Step 7b（§7の06の記録責任・作業台帳・取消／リスト整理の境界）、Step 7c（KMLの位置づけ・単位・生成契機・内容・再送）、Step 7d（PDFの役割分離・生成契機、飛行履歴・出力の画面）、Step 7e（§9の正本・端末cache・時点分離・費用の境界）、Step 8（§11の差分監査）まで。C1前docs再編・本Stepの完了は後続コードの着手承認ではありません。以下§1.2の開始ゲートも適用します。
...
- **実装範囲**:
  ...
  - **KML Geo Export & Google Drive自動保存（詳細は [output README](output/README.md) 参照）**:
    - `KmlExporter`: `FlightAreaGeometry`（Polygon, 近似Circle, Buffered Line）とDIPSへ通報する内容を「1飛行につき1KML」で生成する（運航実績は含めない。[27e](output/27e_kml-generation-timing-and-content.md)）。生成の契機は飛行計画の通報確定で、KmlExporterとDrive保存の実装配分はC8のまま。
    - `GoogleDriveAdapter`: KMLを飛行計画の通報時に保存する（保存先の指定方法は27bのPENDING）。保存できなかったKMLは未同期として保持し、`SyncQueue` 経由で通信復帰時と飛行後点検後の最後の送信時に再送する。
