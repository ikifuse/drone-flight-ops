...
**現在の停止位置**:
- **Phase C0構築完了 / Phase C1コード実装は依然として未着手・厳格禁止**。
- **現在進行中の作業**: 設計検討正本（`99.2_設計検討メモ_全論点・根拠・因果統合正本`）から必要な確定判断・仕様・因果・根拠・例外・PENDINGを正式設計書群（`docs/architecture/`）の各責任領域へ適切に反映する段階的マイグレーション（Step 1〜Step 8）。
- **ADR-0010〜0014の扱い**: Step 1において `Proposed` として起票し、各Stepで該当章・既存正本・根拠との整合を確認後、Step 8の監査を経て「Accepted候補」としてオーナーへ提示し、オーナーの明示承認を受けて初めて `Accepted` へ変更する（自動昇格は行わない）。
- **Phase C1実装開始ゲート**: 99.2に基づく設計反映、差分監査、およびオーナー承認が完了するまで、`src/` 配下のコード実装はいかなる理由があっても開始しない。
