...
- **主要責務**:
  ...
  6. **帳票エクスポート**: 国交省取扱要領に基づく飛行日誌PDF/CSVおよび点検整備記録のブラウザ内直接生成。
- **クライアント層が「やってはならないこと」**:
...
- **主要責務**:
  ...
  2. **認証トークン交換・中継（BFF）**: DIPS認可コードフローにおけるToken Endpointとの通信（認可コードとアクセストークン・リフレッシュトークンの交換）およびセキュアCookieセッションの管理。
...
- **バックエンド層が「やってはならないこと」**:
  ...
  - オフライン不可欠機能への依存強制（Workersがダウンしていても、飛行日誌の記録・保存・閲覧・PDF出力は支障なく動作すること）。
