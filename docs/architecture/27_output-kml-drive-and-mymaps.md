...
本ドキュメントは、新アプリ `drone-flight-ops` において、飛行計画および現場運航記録を外部へ出力・可視化・長期保管・通報するための「4大データ境界」と、KML生成、Google Drive自動保存、およびGoogle My Maps（マイマップ）連携の詳細仕様を定義します。
...
│ ・PDF生成元・検索・集計 │ │ ・人間向け印刷フォーマット│ │ ・1計画1KML・視覚的確認 │
...
## 1.2 DIPS API JSON境界とユーザー非公開の原則

### 1.2.1 ユーザー向けJSONファイル出力の排除
- 一般のドローン操縦者・業務ユーザーにとって、DIPS API仕様の複雑なJSON（No.1〜88の階層オブジェクト）を直接ファイルとして保存・確認・アップロードすることは実用上困難であり、誤認や混乱の原因となります。
- したがって、**新アプリではユーザー向け機能として「DIPS用JSONファイルのエクスポート/インポート」は一切提供しません**。
- ユーザー向けの地図・空間情報出力は **KML** に一本化し、台帳データは **Google Sheets**、法的記録は **PDF** で完結させます。

### 1.2.2 DIPS API JSON のライフサイクルと局所性
- DIPS API JSON は、`ApiDipsAdapter` が国交省サーバー（またはWorkers Proxy）へ `POST /api/v1/flight-plans` を実行する直前に、メモリ上で一時的にシリアライズされる転送用データ（Wire Format）です。
- 通報完了後は、監査・照合目的で `api_payload_snapshot` として内部DBへ保存可能ですが、ファイルとして端末ストレージやDriveへエクスポートする対象ではありません。

---

## 1.3 Submission Snapshot（意味論的通報スナップショット）と API JSON の分離

「手動通報でも保持すべき通報内容の不変記録」と「API通信時のexact JSON」を概念上明確に分離します。
...
│ DipsSubmission                                           │
│                                                          │
│  1. submission_snapshot (必須・意味論的不変Snapshot)     │
│     - 通報方式（manual / api / mock）を問わず保持        │
│     - 提出確定時点の機体、操縦者、目的、空域、方法、     │
│       高度、幾何形状、保険、許可情報等の不変データ       │
│     - 手動入力支援画面（ManualDipsAdapter）の表示・コピー│
│       および Google Sheets 飛行計画台帳へ同期            │
│                                                          │
│  2. api_payload_snapshot (任意・exact outbound JSON)     │
│     - ApiDipsAdapter 経由時のみ生成 (nullable)           │
│     - 国交省APIへ実際にPOSTしたJSON文字列 (C7 Optional)  │
│     - API未利用時・手動通報時は null                     │
└──────────────────────────────────────────────────────────┘
```

- **`submission_snapshot`**:
  - APIの利用可否に関わらず、提出確定（`SNAPSHOT_SAVED`）時点で必ず生成される不変オブジェクト。
  - 手動入力支援画面（`ManualDipsAdapter`）が参照し、1タップコピーUIを展開するための元データとなります。
- **`api_payload_snapshot`**:
  - `ApiDipsAdapter` を通じて実際にAPIリクエストが送出された場合のみ記録される exact outbound JSON。
  - API資格未取得時や手動通報時は存在せず（null）、アプリの完成や台帳記録のブロッカーになりません。
- ※内部DB（IndexedDB）において `submission_snapshot` を技術的にJSON型やシリアライズ形式で永続化することは内部実装の詳細であり、これを「ユーザー向けJSONファイル出力」と混同してはなりません。
