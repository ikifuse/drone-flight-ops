...
[drone-flight-ops]
       │
       ▼ (KML確定生成 ＆ Google Drive自動保存: 07_出力_PDF・KML/KML/)
[Google Drive 指定フォルダ]
       │
...
[Google My Maps 操作]
  ...
  4. 飛行範囲・計画通報属性の表示を確認（事後運航実績は含まない。再現性は検証待ち）
```
...
- **`PENDING-MYMAPS-04: 同一KML再インポート時の更新/重複挙動`**:
  - 計画修正やリビジョン変更等で更新されたKMLを同一マップへ再度インポートした際、既存レイヤーが上書き置換されるか、別レイヤーとして重複追加されるか（※運航完了時の事後実績自動更新はADR-0013により廃止されたため、本検証は計画修正時等のシナリオを対象とする）。
