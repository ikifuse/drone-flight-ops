...
## 8. 通報の実操作と疑似成功の境界（2026-09-23）

**CURRENT-ACCEPTED（今回のオーナー指示）**: [25b §1.1](../dips-flight-plan/25b_manual-web-mapping.md#11-通報時に不足している登録情報を補う受け皿2026-09-23)と34a §8.3で不足だけを補い、入力済み計画・Geometryを保持したまま同じ飛行計画へ戻る。通報内容確認から［通報する］を押して正常受付後の選択へ接続する。確認画面から［直す］で戻っても同じ計画を使う。

**HISTORICAL**: 旧モックの送信中表示は、右側で結果を選ばなければ停止していた。利用者側の操作だけで完了できるよう、既存 `nf-send` を確認に使い、`nf-submit` から正常受付を疑似実行する。異常系の結果選択は右側に残す。

**EVIDENCE/EXAMPLE**: `commitPlan('clean')` は同じ計画の写しを環境の `plans` に追加し、DIPS通報済み（`dips='clean'`）、KML保存済み（オンライン成功時 `kml='saved'`）、共有飛行リスト掲載済みをメモリ内で表す。KMLファイル生成・Drive保存・他端末への共有は行わない。KMLの正式保存先・命名・XMLの決定は含めない（正本は[27e](../output/27e_kml-generation-timing-and-content.md)）。

［飛行前点検へ］は対象計画から点検へ進む。［後で飛行する］は計画を残してホームへ戻る。飛行リスト→対象計画→通報内容→点検は、保存済みの同じ計画を再利用する。§7の重複・結果不明・エラーの停止、Manual確認だけでは通常運航へ進めない境界と、再開方法のPENDINGは変えない。本番では送信ボタンの押下だけを受付・保存成功としない（§2）。
