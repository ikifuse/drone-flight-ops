...
今回の内容は、既存のOperationalEnvironmentの表示・切替と、ホームの4入口を維持したまま、［各種設定・管理］から機体とBATの関係を管理できるようにすることである。2026-09-22の訂正（[34a §7.8](34a_setup-and-environment-entry.md#78-ホームへ入る前を最小にした訂正2026-09-22)）で、［各種設定・管理］は、自分の情報・機体・人員・許可承認・保険・連絡先・DIPSのログイン情報などを**必要になる前に登録しておく入口**でもある。ホームへ入る前に一括で登録させる構成は採らない。画面ごとの10項目は[30の規約](30_screen-specification-standard.md)に沿い、未確定は`PENDING`とする。
