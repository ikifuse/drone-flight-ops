...
| ホームを開いた時 | 複数環境に所属する場合は現在環境と切替手段を明示。**所属が1つなら切替の操作を出さない**（環境名の表示は取り違え防止のため残す。31a §4・[34a §9.6](34a_setup-and-environment-entry.md#96-複数環境と切替)） | まだ選択されていない場合がある |
