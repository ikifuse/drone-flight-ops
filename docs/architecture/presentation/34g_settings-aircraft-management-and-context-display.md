...
今回の内容は、既存のOperationalEnvironmentの表示・切替と、ホームの4入口を維持したまま、［各種設定・管理］から機体とBATの関係を管理できるようにすることである。画面ごとの10項目は[30の規約](30_screen-specification-standard.md)に沿い、未確定は`PENDING`とする。
