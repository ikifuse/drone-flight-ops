...
2026-09-23の同期では、ホームの現在環境表示を押せる入口へ統合した。単一環境でも現在地確認・会社追加を開ける。現在環境のボタン・シートの正本は[34a §9.6](34a_setup-and-environment-entry.md#96-複数環境と切替)であり、ホームの大きな役割／Googleアカウント情報カードや別の切替ボタンは置かない。以下の対象機体の表示責任と混同しない。
...
| ホームを開いた時 | 複数環境に所属する場合は現在環境と切替手段を明示。**所属が1つでも現在地確認・会社追加のボタンを有効にする**（切替候補だけは複数環境時に表示する。31a §4・[34a §9.6](34a_setup-and-environment-entry.md#96-複数環境と切替)） | まだ選択されていない場合がある |
