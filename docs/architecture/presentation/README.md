...
最終更新: 2026-09-21\
範囲: Step 1の10項目規約、Step 2の人物・環境・権限への接続、Step 5の§3と§7の通常画面・共有リスト限定部分、Step 7dの飛行履歴・出力、2026-09-21の利用者向け表示名・文言と内部設計用語の分離。C1未着手。

| 正本 | 責任 |
|---|---|
| [30_screen-specification-standard](30_screen-specification-standard.md) | 画面ごとの記録10項目、導入理由、保存先・権限・通信状態との対応の記録方法、内部設計用語と利用者表示文言の分離規約（§4） |
| [34a 初回・環境への入口](34a_setup-and-environment-entry.md) | 作成／参加、root重複防止、初回必須登録の変化、通常起動・環境選択、アカウントから始める初回導線と利用者向けの表示名の訂正（§7） |
...
| [34h 利用者向けの表示名・文言](34h_user-facing-wording-and-terminology.md) | 内部の概念名と利用者向け表示（案）の対応、画面に出さない語、維持する言葉（現場用語・DIPS公式名）、保存状態・エラー・通報方式の文言の型 |
