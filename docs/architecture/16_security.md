...
§1〜§9が扱うのは、DIPS APIの`client_secret`やToken、固定バックエンドの秘密など、**アプリ固有の機密**である。利用者自身がDIPSへログインするためのID・パスワードは、これとは別の秘密であり、これまで扱っていなかった。オーナーの2026-09-22の指示で、現時点の方針は、アプリがDIPSログインID・パスワードを登録・保存できることとした（画面と方針は[34a §8](presentation/34a_setup-and-environment-entry.md#8-dipsログイン情報の登録と初回設定の候補2026-09-22)。CURRENT-PROPOSAL）。

2026-09-23に、オーナーの指示で、DIPSログインID・パスワードは**初回登録（はじめの登録）で本人情報と同じ1画面で登録する**こととした（[34a §9.3](presentation/34a_setup-and-environment-entry.md#93-初回登録は1画面にまとめる)・[ADR-0030](../decisions/ADR-0030-personal-first-onboarding-and-single-screen-initial-registration.md)）。飛行現場で通報の直前に設定へ寄り道させないためである。保存できるという方針は変わらず、**保存方式が未決であることも変わらない**。初回から扱う範囲が広がるため、リスクの評価は保存方式を決める時点のオーナー判断に残す。

**PENDING-S5-DIPS-LOGIN-STORAGE**: 保存先、暗号化の方式、端末ごとか共有か、Google Drive／Sheetsでの持ち方、複数人が使うときに誰が見られるか、自動ログインに使うか、API認証との関係、DIPS公式ログイン画面への自動入力の可否、アクセストークン／リフレッシュトークン等の具体実装は未決である。方針は変えず、§1の「クライアントへ機密を出さない」原則との緊張関係は、保存方式を決める際のオーナー判断の材料として残す。§5のログ・エクスポートからのマスキングは、この情報にも適用する方向で確認する（未決）。公開Docsやログ例へ、実際のID・パスワードを入れない（§7）。
