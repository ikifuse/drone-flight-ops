...
**PENDING-S5-DIPS-LOGIN-STORAGE**: 保存先、暗号化の方式、端末ごとか共有か、Google Drive／Sheetsでの持ち方、複数人が使うときに誰が見られるか、自動ログインに使うか、API認証との関係は未決である。方針は変えず、§1の「クライアントへ機密を出さない」原則との緊張関係は、保存方式を決める際のオーナー判断の材料として残す。§5のログ・エクスポートからのマスキングは、この情報にも適用する方向で確認する（未決）。公開Docsやログ例へ、実際のID・パスワードを入れない（§7）。
