...
## 10. 利用者自身のDIPSログイン情報（2026-09-22。方針のみ）

§1〜§9が扱うのは、DIPS APIの`client_secret`やToken、固定バックエンドの秘密など、**アプリ固有の機密**である。利用者自身がDIPSへログインするためのID・パスワードは、これとは別の秘密であり、これまで扱っていなかった。オーナーの2026-09-22の指示で、現時点の方針は、アプリがDIPSログインID・パスワードを登録・保存できることとした（**CURRENT-ACCEPTED。方針のみ**。画面と方針の正本は[34a §8.1](presentation/34a_setup-and-environment-entry.md#81-発端方針未決)、判断の要約は[ADR-0030](../decisions/ADR-0030-personal-first-onboarding-and-single-screen-initial-registration.md) Proposed）。**HISTORICAL**: 2026-09-22時点の本節は、この方針をCURRENT-PROPOSALと表記していた。34a §8.1・§9.3で方針が確定した後も残っていた古い表記を、2026-09-25に訂正した。保存方式は下記のとおり未決で、この訂正によって確定しない。

2026-09-23に、オーナーの指示で、DIPSログインID・パスワードは**初回登録（はじめの登録）で本人情報と同じ1画面に置き、任意で登録できる**こととした（必須は氏名とGoogleアカウントの2つだけ。未登録のまま進んだ場合は通報の直前にその場で登録できる。[34a §9.3](presentation/34a_setup-and-environment-entry.md#93-初回登録は1画面にまとめる)・[ADR-0030](../decisions/ADR-0030-personal-first-onboarding-and-single-screen-initial-registration.md)）。飛行現場で通報の直前に設定へ寄り道させないためである。保存できるという方針は変わらず、**保存方式が未決であることも変わらない**。初回から扱う範囲が広がるため、リスクの評価は保存方式を決める時点のオーナー判断に残す。

**PENDING-S5-DIPS-LOGIN-STORAGE**: 保存先、暗号化の方式、端末ごとか共有か、Google Drive／Sheetsでの持ち方、複数人が使うときに誰が見られるか、自動ログインに使うか、API認証との関係、DIPS公式ログイン画面への自動入力の可否は未決である。DIPS APIの正式認証仕様・credential・endpoint・失効・照合APIの条件は§9のVERIFY-S4-API-CONTRACT、アクセストークン／リフレッシュトークン等のToken／Session保持は§9のPENDING-S4-SESSIONであり、本IDとは別に扱う。方針は変えず、§1の「クライアントへ機密を出さない」原則との緊張関係は、保存方式を決める際のオーナー判断の材料として残す。§5のログ・エクスポートからのマスキングは、この情報にも適用する方向で確認する（未決）。公開Docsやログ例へ、実際のID・パスワードを入れない（§7）。
